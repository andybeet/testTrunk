				...
				if (FunctGroupArray[sp].isVertebrate == TRUE) {
					...
					for (n = 0; n < FunctGroupArray[sp].numCohortsXnumGenes; n++) {
						for (ij = MIGRATION[sp].num_in_queue_done; ij < MIGRATION[sp].num_in_queue; ij++) {
                            returning_biom = (MIGRATION[sp].SN[n][ij] + MIGRATION[sp].RN[n][ij]) * MIGRATION[sp].DEN[n][ij];
                            if (returning_biom > 0){
                                bm->totbiom[sp] += returning_biom;
                                bm->totagepop[sp][n] += returning_biom;
                            }
							...
							**/
							...
							if(bm->totbiom[sp] < 0){
                                printf("queue entry (ij): %d migration SN = %e, RN = %e, DEN = %e\n", ij, MIGRATION[sp].SN[n][ij], MIGRATION[sp].RN[n][ij], MIGRATION[sp].DEN[n][ij]);
...
void Write_Migration_File(FILE *fid, MSEBoxModel *bm){
    ...
    for (sp=0; sp<bm->K_num_tot_sp; sp++) {
    	...
    	if ( (int)(FunctGroupArray[sp].speciesParams[flag_id]) && (int)FunctGroupArray[sp].speciesParams[num_migrate_id] > 0){
            spmigrate = MIGRATION[sp].num_in_queue;
            spmigrate_done = MIGRATION[sp].num_in_queue_done;
            numValues = (int)(spmigrate /  (int)FunctGroupArray[sp].speciesParams[num_migrate_id]);

           	if(FunctGroupArray[sp].isVertebrate == TRUE){

           		age_mat = (int) (FunctGroupArray[sp].speciesParams[age_mat_id]);
           		for(i = 0; i < (int)FunctGroupArray[sp].speciesParams[num_migrate_id]; i++){
                    ...
                    for (j = 0; j < numValues; j++){
                        ...
                        index = (j * FunctGroupArray[sp].speciesParams[num_migrate_id]) + i;
                        for(n = 0; n < FunctGroupArray[sp].numCohortsXnumGenes; n++){
...
FILE * Init_MigrationDump_File(MSEBoxModel *bm)
{
    ...
    /** Column definitions **/
    fprintf(fid,"Time Species Cohort MigID DEN RN SN Leave Return ReturnCohort\n");
...
void Dump_Migration_Array(FILE *fid, MSEBoxModel *bm){
    int sp, n, i, migID, return_stage, leave_stage;
        ...
        if ( (int)(FunctGroupArray[sp].speciesParams[flag_id]) && (int)FunctGroupArray[sp].speciesParams[num_migrate_id] > 0){
            for(n = 0; n < FunctGroupArray[sp].numCohortsXnumGenes; n++){
                for(i = 0; i < MIGRATION[sp].num_in_queue; i++){
                    migID = MIGRATION[sp].migrationID[n][i];
                    return_stage = MIGRATION[sp].return_stage_prm[n][migID];
                    leave_stage = MIGRATION[sp].leave_stage_prm[n][migID];
                    
                    fprintf(fid,"%e %s %d %d %e %e %e %d %d %d\n", bm->dayt, FunctGroupArray[sp].groupCode, n, i, MIGRATION[sp].DEN[n][i], MIGRATION[sp].SN[n][i], MIGRATION[sp].RN[n][i], MIGRATION[sp].Leave_Now[leave_stage][i], MIGRATION[sp].Return_Now[return_stage][i], MIGRATION[sp].start_return_chrt[n][i]);
                }
            }
        }
    }
