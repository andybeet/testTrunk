...
void injection(MSEBoxModel *bm, Box *bp, SedModel *sm, double **newwc, double **newsed) {
    ...
    if (sum < small_num) {
		free1d(rate);
		return;
    } else if (bm->dayt > 34100.0) {
        fprintf(logfp, "Time: %e box %d bioirrg sum: %e\n", bm->dayt, bp->n, sum);
    }
    ...
    for (k = tk; k < sm->nz; k++) {
        ...
        if (bm->dayt > 34100.0) {
            fprintf(logfp, "Time: %e box %d-%d bioirrg rate: %e irate: %e sum: %e\n", bm->dayt, bp->n, k, rate[k], irate, sum);
        }
    }
