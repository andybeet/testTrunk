...
//	bm->checkstop = (int) Util_XML_Read_Value(fileName, ATLANTIS_ATTRIBUTE, bm->ecotest, 1, groupingNode, integer_check, "checkstop");
	...
	if(bm->track_contaminants == TRUE){
        ...
        bm->flag_contamMortModel = (int) Util_XML_Read_Value(fileName, ATLANTIS_ATTRIBUTE, bm->ecotest, 1, groupingNode, integer_check, "flag_contamMortModel");
        bm->flag_contamInteractModel = (int) Util_XML_Read_Value(fileName, ATLANTIS_ATTRIBUTE, bm->ecotest, 1, groupingNode, integer_check, "flag_contamInteractModel");
        bm->flag_contamGrowthModel = (int) Util_XML_Read_Value(fileName, ATLANTIS_ATTRIBUTE, bm->ecotest, 1, groupingNode, integer_check, "flag_contamGrowthModel");
		...
		bm->contaminantStructure = (ContaminantStructure **)malloc(sizeof(ContaminantStructure *));
		...
		/* Read in the names */
		if(Util_XML_Read_Array_String(ATLANTIS_ATTRIBUTE, fileName, "contaminant_names", groupingNode, "contaminant_names", &values,
