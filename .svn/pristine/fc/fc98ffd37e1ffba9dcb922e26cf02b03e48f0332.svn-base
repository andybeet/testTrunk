/*********************************************************************
 ...
 Purpose:        Map projection routines.
 If specified the PROJ4 library maybe specified.

 Revisions:      none

 $Id: mapproj.c 3570 2012-12-19 08:31:08Z gor171 $
 ...
 Added support for projections in the bgm input file. These projections are
 used to accurately calculate the number of sun light hours.


 07-10-2009 Bec Gorton
 Added better error checking for the projection strings.
...
static char* get_argument_value(char* tag, int nargs, char* arglist[], int req);
static char** add_argument(char *arg, int nargs, char *args[], int *new_nargs, int freeOld);
static void subst_argument_value(char *arg, char* value, int nargs, char *args[]);

static void* merc_init(struct MapProjection *mp, int nargs, char *args[]);
static void merc_free(void *data);
static void merc_forward(struct MapProjection *mp, double lat, double lon, double *east, double *north);
static void merc_inverse(struct MapProjection *mp, double east, double north, double *lat, double *lon);

static void* tcc_init(struct MapProjection *mp, int nargs, char *args[]);
static void tcc_free(void *data);
static void tcc_forward(struct MapProjection *mp, double lat, double lon, double *east, double *north);
static void tcc_inverse(struct MapProjection *mp, double east, double north, double *lat, double *lon);
static double tcc_footpoint_latitude(struct MapProjection *mp, double M);

static void* lcc_init(struct MapProjection *mp, int nargs, char *args[]);
static void lcc_free(void *data);
static void lcc_forward(struct MapProjection *mp, double lat, double lon, double *east, double *north);
static void lcc_inverse(struct MapProjection *mp, double east, double north, double *lat, double *lon);

static void* serov_init(struct MapProjection *mp, int nargs, char *args[]);
static void serov_free(void *data);
static void serov_forward(struct MapProjection *mp, double lat, double lon, double *east, double *north);
static void serov_inverse(struct MapProjection *mp, double east, double north, double *lat, double *lon);

#if defined(PROJ4)
static void* proj_init(struct MapProjection *mp, int nargs, char *args[]);
static void proj_free(void *data);
static void proj_forward(struct MapProjection *mp, double lat, double lon, double *east, double *north);
static void proj_inverse(struct MapProjection *mp, double east, double north, double *lat, double *lon);
#endif
...
MapProjection *mpInitialise(int nargs, char *args[]) {
	...
	char errorString[4000];

	/* Check for those projections that can be handled outside of 'proj'. */

	/* Australian Map Grid */
	if (strcasecmp(projname, "amg") == 0) {
		new_args = add_argument("ellps=agd66", nargs, args, &new_nargs, FALSE);
		new_args = add_argument("south", new_nargs, new_args, &new_nargs, TRUE);
		subst_argument_value("proj", "utm", new_nargs, new_args);
		mp = mpInitialise(new_nargs, new_args);
	...
	/* Map Grid of Australia */
		...
		new_args = add_argument("ellps=gda94", nargs, args, &new_nargs, FALSE);
		new_args = add_argument("south", new_nargs, new_args, &new_nargs, TRUE);
		subst_argument_value("proj", "utm", new_nargs, new_args);
		mp = mpInitialise(new_nargs, new_args);
		for (i = 0; i < new_nargs; i++) {
	...
	/* Universal Transverse Mercator */
		...
		new_args = add_argument("x_0=500000.0", nargs, args, &new_nargs, FALSE);

		if (is_north)
			new_args = add_argument("y_0=0.0", new_nargs, new_args, &new_nargs, TRUE);
		else
...
//		free(new_args);
	...
	/* Mercator, Transverse Mercator, Lambert Conformal Conic
	 * and CMR remote sensing Serov projection. */
	else if ((strcasecmp(projname, "merc") == 0) || (strcasecmp(projname, "tcc") == 0) || (strcasecmp(projname, "lcc") == 0)
			|| (strcasecmp(projname, "serov") == 0)) {
		int got_ellip_data = 0;
		char* ename;
		char* false;
		mp = (MapProjection *) malloc(sizeof(MapProjection));
		...
		if (ename != NULL) {
			struct Ellipsoid* e = &ellipsoids[0];
			while (e->id != NULL) {
		...
		false = get_argument_value("x_0", nargs, args, 0);
		if (false != NULL)
			mp->falsex = atof(false);
		false = get_argument_value("y_0", nargs, args, 0);
		if (false != NULL)
			mp->falsey = atof(false);
		...
		if (mp->private_data == NULL) {
			int i;

			strcpy(errorString, "");
			for (i = 0; i < nargs; i++) {
				strcat(errorString, args[i]);
			}
			fprintf(stderr,
					"\n\nERROR: There is a problem with the projection string in your bgm file.\n\n'%s' \n\nPlease look on the wiki for projection string information.\n",
					errorString);
			fprintf(stderr, "The projection string should be a set of space separated values.\n\n");
			quit("ERROR\n");
		}
 ...
 */
void mpForward(MapProjection *mp, double lat, double lon, double *east, double *north) {
 ...
 * @param long Pointer to longitude in degrees (-180 to 180).
 */
void mpInverse(MapProjection *mp, double east, double north, double *lat, double *lon) {
 ...
 */
static char *get_argument_value(char *tag, int nargs, char *args[], int req) {
...
static char** add_argument(char *arg, int nargs, char *args[], int *new_nargs, int freeOld) {
	...
	*new_nargs = nargs + 1;
	//new_args = c_alloc2d((*new_nargs), STRLEN);
...
//	}
    ...
    new_args = (char**)malloc(sizeof(char*)*(size_t)(*new_nargs));
    new_args[0] = (char *)malloc(sizeof(char *)*strlen(arg)+1);
    strcpy(new_args[0], arg);
 ...
 */
static void *merc_init(struct MapProjection *mp, int nargs, char *args[]) {
	MercData *data = (MercData *) malloc(sizeof(MercData));
 ...
 */
static void merc_forward(struct MapProjection *mp, double lat, double lon, double *east, double *north) {
	MercData *data = (MercData *) mp->private_data;
...
static void merc_inverse(struct MapProjection *mp, double east, double north, double *lat, double *lon) {
	...
	double phi = HALF_PI - 2.0 * atan(exp(-(north - mp->falsey) / mp->ellip_major));
...
static void *tcc_init(struct MapProjection *mp, int nargs, char *args[]) {
	...
	TCCData *data = (TCCData *) malloc(sizeof(TCCData));
	...
	data->mdF0 = mp->ellip_major * (1.0 - data->e2 / 4.0 - (3.0 / 64.0) * data->e4 - (5.0 / 256.0) * data->e6);
	data->mdF1 = 1.0 - data->e2 / 4.0 - (3.0 / 64.0) * data->e4 - (5.0 / 256.0) * data->e6;
	data->mdF2 = (3.0 / 8.0) * data->e2 + (3.0 / 32.0) * data->e4 + (45.0 / 1024.0) * data->e6;
 ...
 */
static void tcc_forward(struct MapProjection *mp, double lat, double lon, double *east, double *north) {
	TCCData *data = (TCCData *) mp->private_data;
	...
	if (rlat != HALF_PI && rlat != -HALF_PI) {
		...
		x = A + (1.0 - T + C) * A3 / 6.0 + (5.0 - 18.0 * T + T2 + 72.0 * C - 58.0 * data->edash2) * A5 / 120.0;
		x *= data->k0 * N;
		y = N * tanlat * (A2 / 2.0 + (5.0 - T + 9.0 * C + 4.0 * C2) * A4 / 24.0 + (61.0 - 58.0 * T + T2 + 600.0 * C - 330 * data->edash2) * A6 / 720.0);
	}
...
static void tcc_inverse(struct MapProjection *mp, double east, double north, double *lat, double *lon) {
	...
	double tanlat = sinlat / coslat;
	double N = mp->ellip_major / sqrt(1 - data->e2 * sinlat2);
	double R = mp->ellip_major * (1 - data->e2) / pow(1 - data->e2 * sinlat2, 3.0 / 2.0);
	double T = tanlat * tanlat;
	double T2 = T * T;
	double C = data->edash2 * coslat2;
	double C2 = C * C;
	...
	double D5 = D4 * D;
	double D6 = D5 * D;
	double tlat = (N * tanlat / R)
			* ((D2 / 2.0 - (5.0 + 3 * T + 10 * C - 4 * C2 - 9 * data->edash2) * D4 / 24.0
					+ (61 + 90 * T + 298 * C + 45 * T2 - 252 * data->edash2 - 3 * C2) * D6 / 720.0));
	double tlon = (D - (1. + 2 * T + C) * D3 / 6.0 + (5 - 2 * C + 28 * T - 3 * C2 + 8 * data->edash2 + 24 * T2) * D5 / 120.0) / coslat;
...
static void *lcc_init(struct MapProjection *mp, int nargs, char *args[]) {
	double oA, oB, oC, oD;
	LCCData *data = (LCCData *) malloc(sizeof(LCCData));
	data->cm = DEG2RAD(CENTRAL_MERIDIAN(nargs, args));
	...
	data->m1 = data->cosphi1 / sqrt(1 - data->e2 * data->sinphi1 * data->sinphi1);
	data->m2 = data->cosphi2 / sqrt(1 - data->e2 * data->sinphi2 * data->sinphi2);
	data->t1 = sqrt(((1 - data->sinphi1) / (1 + data->sinphi1)) * pow((1 + data->e * data->sinphi1) / (1 - data->e * data->sinphi1), data->e));
	data->t2 = sqrt(((1 - data->sinphi2) / (1 + data->sinphi2)) * pow((1 + data->e * data->sinphi2) / (1 - data->e * data->sinphi2), data->e));
	...
	data->t0 = sqrt(((1 - data->sinphi0) / (1 + data->sinphi0)) * pow((1 + data->e * data->sinphi0) / (1 - data->e * data->sinphi0), data->e));
	data->F = data->m1 / (data->nn * pow(data->t1, data->nn));
	data->rho0 = mp->ellip_major * data->F * pow(data->t0, data->nn);

	oA = data->e2 / 2.0 + 5.0 * data->e4 / 24.0 + data->e6 / 12.0 + 13.0 * data->e8 / 360.0;
	oB = 7.0 * data->e4 / 48.0 + 29.0 * data->e6 / 240.0 + 811.0 * data->e8 / 11520.0;
 ...
 */
static void lcc_forward(struct MapProjection *mp, double lat, double lon, double *east, double *north) {
	LCCData *data = (LCCData *) mp->private_data;
	double rlat = DEG2RAD(lat);
	double rlon = DEG2RAD(lon);
	double theta = data->nn * (rlon - data->cm);
	double sinlat = sin(rlat);
	double esinlat = data->e * sinlat;
	double t = sqrt(((1 - sinlat) / (1 + sinlat)) * pow((1 + esinlat) / (1 - esinlat), data->e));
...
static void lcc_inverse(struct MapProjection *mp, double east, double north, double *lat, double *lon) {
	...
	double rlat = phi + sinphi * (data->A + cosphi * (data->B + cosphi * (data->C + data->D * cosphi)));
...
static void *serov_init(struct MapProjection *mp, int nargs, char *args[]) {
	...
	new_args = add_argument("lon_0=150", nargs, args, &new_nargs, FALSE);
	new_args = add_argument("lat_1=-40", new_nargs, new_args, &new_nargs, TRUE);
	new_args = add_argument("lat_2=-10", new_nargs, new_args, &new_nargs, TRUE);
...
/* Never used
 static void *proj_init_plus(struct MapProjection *mp, char *str)
 {
 return pj_init_plus(str);
 }
 */
...
static void proj_forward(struct MapProjection *mp, double lat, double lon, double *east, double *north) {
	...
	lp.u = DEG2RAD(lon);
	lp.v = DEG2RAD(lat);
	xy = pj_fwd(lp, (projPJ *) mp->private_data);
	*east = xy.u;
	*north = xy.v;
}
...
static void proj_inverse(struct MapProjection *mp, double east, double north, double *lat, double *lon) {
	...
	pj_latlong = pj_latlong_from_proj(mp->private_data);

	if (pj_latlong == NULL)
		quit("Unable to set up lat/long projection parameters");

	x = east;
	y = north;

	if ((returnValue = pj_transform(mp->private_data, pj_latlong, 1, 1, &x, &y, NULL)) != 0) {
		printf("east = %f\n", east);
		printf("north = %f\n", north);

		printf("pj_get_def(pj_latlong) = %s\n", pj_get_def(pj_latlong, 0));
		printf("pj_get_def(mp->private_data) = %s\n", pj_get_def(mp->private_data, 0));
		quit("pj_transform did not return a sucess\n");
	}
