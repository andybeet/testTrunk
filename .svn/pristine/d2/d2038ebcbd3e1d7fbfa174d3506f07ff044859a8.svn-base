/**
 \file
 \brief C file for running the movement routines in south east.
 \ingroup atecology
 File:		atmovement.c
 Created:	21/9/2004
 Author:		Beth Fulton,   CSIRO Marine Laboratories,  2004
 Based on work by Ouyang and Fulton for Port Phillip Bay
 Purpose:	C file for running the movement routines in south east.
...
void Ecology_Total_Verts_And_Migration(MSEBoxModel *bm, double dt, FILE *llogfp) {
	int ij, k, n, qrt, sp, flagmother, flagsp, /*spjmin, spjmax, spamin =
	 0, spamax,*/age_mat, sp_ddepend_move, day_part, localised_recruit, sn, rn, den, mig_done, spmigrate, spmigrate_done, bearlive, stage,
			flagimposecatch, stock_id = -1, spupdatemig, do_debug, recalc_needed, decdis, sp_feed_while_spawn, sp_spawn_now, nmigs, nn, kn,
			flagchannel, desired_chrt, nf, flagcontract_sp, qid, ngene, spawn_period, spawn_date, bcohort, nc, start_return_chrt, end_return_chrt,
			temp_sensitive_sp, salt_sensitive_sp, update_larval_distrib, updated_already, test_day, migID = 0, return_stage = 0, leave_stage = 0;
	double ldayt = bm->t / 86400.0;
	//double migtemp = 86400.0 / dt;
	int maxij = bm->K_num_stocks_per_sp;
	int overall_checkday = (int) (floor(bm->dayt));

	double clear, E1_sp, spSpeed, vertdistrib, mignum, mignum_actual, spawnmove = 1.0, totmig, midpoint, step1, step2, mignum_nc, this_totmig,
			FSMG_grow, migtime, oldden, totdenom, min_temp_sp, max_temp_sp, maxstock, min_spawntemp_sp, max_spawntemp_sp, temp_effect, finalmig,
			avgsn, avgrn, dynsn, dynrn, min_salt_sp, max_salt_sp, min_O2_sp, current_enviro, min_spawnsalt_sp, max_spawnsalt_sp, salt_effect,
			o2_effect, contract_sp, oldsn, oldrn, this_tot_biom, pH_scale, orig_newden, check_day, prop_nc, stagger_scalar, this_mig_num;
	int enviro_depend = 0, sp_enviro_depend = 0;
	int **counted;
	int rij, rangeid, rocstage = -1, thiscase1, thiscase2, adstage, snn, rnn, sp_Migrate_Years, do_stagger = 0, stagger_return, NumStagger;
	double nbox_spread;
	int adbox, chkbox;
	double prop_range;
	int prij, rrid;
	...
	Util_Init_3D_Double(bm->HomeRangeTotal, bm->K_num_homerange, bm->K_num_max_cohort * bm->K_num_max_genetypes, bm->K_num_tot_sp, 0.0);
	...
	**/
	...
	for (sp = 0; sp < bm->K_num_tot_sp; sp++) {
		...
		if (FunctGroupArray[sp].isVertebrate == TRUE) {
            ...
            nmigs = (int)(FunctGroupArray[sp].speciesParams[num_migrate_id]);
            ...
            for (n = 0; n < FunctGroupArray[sp].numCohortsXnumGenes; n++) {
                ...
                for (qid = spmigrate_done; qid < spmigrate; qid++) {
                    ...
                    if(migID > (nmigs - 1))
                        quit("In loop over migrants, trying to call a migration ID (%d) that exceeds queue length (%d) for %s\n", migID, nmigs, FunctGroupArray[sp].groupCode);
                    return_stage = MIGRATION[sp].return_stage_prm[n][migID];
                    leave_stage = MIGRATION[sp].leave_stage_prm[n][migID];
                    start_return_chrt = MIGRATION[sp].start_return_chrt[n][qid];
                    end_return_chrt = MIGRATION[sp].end_return_chrt[n][qid];
                    stock_id = (int) (MIGRATION[sp].returnstock[return_stage][migID]) - 1;

					/**/
                    
                   if(do_debug2){
                   // if (sp == 44) {
                        fprintf(llogfp, "Time: %e, Species %s-%d (stage %d) qid: %d Return_Period = %d, Return_Now: %d (num_in_queue: %d, num_in_queue_done: %d)\n",
							bm->dayt, FunctGroupArray[sp].groupCode, n, return_stage, qid, MIGRATION[sp].Return_Period[return_stage][migID], MIGRATION[sp].Return_Now[return_stage][qid], MIGRATION[sp].num_in_queue, MIGRATION[sp].num_in_queue_done);
					}
                    /**/

                    
                    if (((MIGRATION[sp].Return_Period[return_stage][migID] > 0) && (bm->dayt < MIGRATION[sp].Return_Now[return_stage][qid])) && (FunctGroupArray[sp].isComplexMigrator == FALSE))
                        break;

					/* Migrating species returning to the model */
					if ((spmigrate && (MIGRATION[sp].Return_Period[return_stage][migID] > 0)) && (bm->dayt >= MIGRATION[sp].Return_Now[return_stage][qid]) && (bm->dayt < (MIGRATION[sp].Return_Now[return_stage][qid] + MIGRATION[sp].Return_Period[return_stage][migID])) && (bm->t / 86400.0 > 0.0)) {

						/**
						 if(do_debug2){
                             fprintf(llogfp,"Time: %e, %s-%d (return_stage: %d), mig-%d, MigBack: %d, MigPeriod: %d, FinalMigBack: %d\n",
									bm->dayt, FunctGroupArray[sp].groupCode, stage, return_stage, qid,
									MIGRATION[sp].Return_Now[return_stage][qid], MIGRATION[sp].Return_Period[return_stage][migID],
									MIGRATION[sp].Return_Now[return_stage][qid]+MIGRATION[sp].Return_Period[return_stage][migID]);
						 }
						 **/
                       ...
                       /* Update numbers for returning migrants and then update overall size,
						...
						migtime2 = Calculate_Migration_Proportion_Back(bm, llogfp, dt, sp, MIGRATION[sp].Return_Now[return_stage][qid], MIGRATION[sp].Return_Period[return_stage][migID]);
						migtime = MIGRATION[sp].Return_Now[return_stage][qid] - MIGRATION[sp].Leave_Now[leave_stage][qid];

                        /**/
						if(do_debug2){
                        //if (sp == 44) {
                            fprintf(bm->logFile, "Time: %e, %s migtime2 = %e, period = %d, migtime = %e, migrateReturn = %d, away = %d with original migDEN: %e  start_return_chrt: %d, end_return_chrt: %d\n", bm->dayt, FunctGroupArray[sp].groupCode, migtime2, MIGRATION[sp].Return_Period[return_stage][migID], migtime, MIGRATION[sp].Return_Now[return_stage][qid], MIGRATION[sp].Leave_Now[leave_stage][qid], MIGRATION[sp].DEN[n][qid], start_return_chrt, end_return_chrt);
						}
                        /**/
                        ...
                        mignum_actual = MIGRATION[sp].mortality[return_stage][migID] * mignum;
                        step1 = MIGRATION[sp].num_aging_event[n][qid] / ((double)(FunctGroupArray[sp].ageClassSize));
                        step2 = step1 - floor(step1);
                        
                        for (nc = start_return_chrt; nc < end_return_chrt; nc++) {
                            /* Migrants coming in. For all years after the first year the values
                             are based on populations that migrated out of the model domain previously.
                             For migrations back in the first year values from KMIG are used
                             (fed into Migration matrix in biolsetup) */
                            
                            prop_nc = (1.0 - step2);

                            mignum_nc = mignum_actual * prop_nc;
                            oldden = totden[sp][nc];
                            totden[sp][nc] += mignum_nc;
                            mig_returners[sp] = 1;
                        
                            /**/
                            if(do_debug2){
                            //if (sp == 44) {
                                fprintf(bm->logFile,"Time: %e, %s-%d totden: %e with mignum_nc: %e prop_nc: %e step1: %e step2 %e\n", bm->dayt, FunctGroupArray[sp].groupCode, nc, totden[sp][nc], mignum_nc, prop_nc, step1, step2);
                            }
                            /**/
                            ...
                            if (stock_id < 0) {
                                ...
                                for (k = 0; k < maxstock; k++) {
                                    init_stock_struct_prop[sp][nc][k] += (mignum_nc / maxstock);
                                }
                            } else {
                                /* Directed to a specific stock */
                                init_stock_struct_prop[sp][nc][stock_id] += mignum_nc;
                            }

                            /**
                            if(do_debug2){
                                fprintf(llogfp, "Time: %e, sp %s (cohort %d, nc: %d), num returning: %e (mort: %e * mignum: %e)\n",
                                        bm->dayt, FunctGroupArray[sp].groupCode, n, nc, mignum_actual, MIGRATION[sp].mortality[return_stage][migID], mignum_nc);
                            }
                            **/
                            ...
                            oldsn = VERTinfo[sp][nc][SN_id];
                            if(bm->flag_replicated_old)
                                FSMG_grow = 1.0 + (MIGRATION[sp].growth[return_stage][migID] * migtime / 364.0) / FunctGroupArray[sp].X_RS[nc];
                            else
                                FSMG_grow = 1.0 + (MIGRATION[sp].growth[return_stage][migID] * migtime / 364.0);
                        
                            if (MIGRATION[sp].SN[n][qid] > 0)
                                VERTinfo[sp][nc][SN_id] = (oldsn * oldden + MIGRATION[sp].SN[n][qid] * FSMG_grow * mignum_nc) / (totden[sp][nc] + small_num);

                            /**/
                            if(do_debug2){
                            //if (sp == 44) {
                                fprintf(llogfp,"Time: %e, %s-%d qid: %d newSN: %e (oldsn: %e, oldden: %e, MigSN: %e, FSMG: %e, mignum_nc: %e, totden: %e, step1: %e, step2: %e, step1+step2: %e, step1+step2/totden): %e\n", bm->dayt, FunctGroupArray[sp].groupCode, nc, qid, VERTinfo[sp][nc][SN_id], oldsn, oldden, MIGRATION[sp].SN[n][qid], FSMG_grow, mignum_nc, (totden[sp][nc] + small_num), (oldsn * oldden), (MIGRATION[sp].SN[n][qid] * FSMG_grow * mignum_nc), (oldsn * oldden + MIGRATION[sp].SN[n][qid] * FSMG_grow * mignum_nc), ((oldsn * oldden + MIGRATION[sp].SN[n][qid] * FSMG_grow * mignum_nc) / (totden[sp][nc] + small_num)));
                                fprintf(llogfp,"%s-%d VERTinfoSN: %e vs calc: %e\n",
									FunctGroupArray[sp].groupCode, nc, VERTinfo[sp][nc][SN_id], ((oldsn * oldden + MIGRATION[sp].SN[n][qid] * FSMG_grow * mignum) / (totden[sp][nc] + small_num)));
                            }
                            /**/

                            oldrn = VERTinfo[sp][nc][RN_id];

                            if (MIGRATION[sp].RN[n][qid] > 0)
                                VERTinfo[sp][nc][RN_id] = (oldrn * oldden + MIGRATION[sp].RN[n][qid] * FSMG_grow * mignum_nc) / (totden[sp][nc] + small_num);
                        }
						...
						if ((bm->dayt + 1) > (MIGRATION[sp].Return_Now[return_stage][qid] + MIGRATION[sp].Return_Period[return_stage][migID])) {
							if (bm->flagavgmig) {
								MIGRATION[sp].SN[n][qid] = 0.0;
								MIGRATION[sp].RN[n][qid] = 0.0;
							}
	...
	for (sp = 0; sp < bm->K_num_tot_sp; sp++) {
		if (FunctGroupArray[sp].isVertebrate == TRUE) {
            ...
            stagger_return = (int)(FunctGroupArray[sp].speciesParams[flag_multiyr_type_id]);
            NumStagger = (int)(FunctGroupArray[sp].speciesParams[stagger_years_id]);
				...
				if(bm->terrestrial_on && bm->boxes[ij].type == LAND){
					...
					if( FunctGroupArray[sp].habitatCoeffs[LAND_BASED] > 0){
						...
						for (n = 0; n < FunctGroupArray[sp].numCohortsXnumGenes; n++) {
							...
							if (qrt < (FunctGroupArray[sp].numMoveEntries - 1))
								newden[sp][n][0][ij] = 1.0 * (bm->HowFar * (FunctGroupArray[sp].distrib[ij][qrt + 1][stage] - FunctGroupArray[sp].distrib[ij][qrt][stage])
										+ FunctGroupArray[sp].distrib[ij][qrt][stage]);
							else
								newden[sp][n][0][ij] = 1.0 * (bm->HowFar * (FunctGroupArray[sp].distrib[ij][0][stage] - FunctGroupArray[sp].distrib[ij][qrt][stage])
										+ FunctGroupArray[sp].distrib[ij][qrt][stage]);
							...
							*/
                            ...
                            switch (sp_ddepend_move) {
                                ...
                                if (thiscase1 || (sp_ddepend_move != switch_ddepend)) {
        							if (qrt < (FunctGroupArray[sp].numMoveEntries - 1))
                                        spawnmove = bm->HowFar * (FunctGroupArray[sp].distrib[ij][qrt + 1][stage] - FunctGroupArray[sp].distrib[ij][qrt][stage])
                                                + FunctGroupArray[sp].distrib[ij][qrt][stage];
                                    else
                                        spawnmove = bm->HowFar * (FunctGroupArray[sp].distrib[ij][0][stage] - FunctGroupArray[sp].distrib[ij][qrt][stage]) + FunctGroupArray[sp].distrib[ij][qrt][stage];
                                } else if (thiscase2){
                            ...
                            case sticky_ddepend:
                                ...
                                /* Find other migration pressure */
    							if (qrt < (FunctGroupArray[sp].numMoveEntries - 1))
                                    spawnmove = bm->HowFar * (FunctGroupArray[sp].distrib[ij][qrt + 1][stage] - FunctGroupArray[sp].distrib[ij][qrt][stage]) + FunctGroupArray[sp].distrib[ij][qrt][stage];
                                else
                                    spawnmove = bm->HowFar * (FunctGroupArray[sp].distrib[ij][0][stage] - FunctGroupArray[sp].distrib[ij][qrt][stage]) + FunctGroupArray[sp].distrib[ij][qrt][stage];
                            ...
                            case perscribed_move:
    							...
    							if (qrt < (FunctGroupArray[sp].numMoveEntries - 1))
                                    newden[sp][n][k][ij] = vertdistrib * (bm->HowFar * (FunctGroupArray[sp].distrib[ij][qrt + 1][stage] - FunctGroupArray[sp].distrib[ij][qrt][stage])
                                            + FunctGroupArray[sp].distrib[ij][qrt][stage]);
                                else
                                    newden[sp][n][k][ij] = vertdistrib * (bm->HowFar * (FunctGroupArray[sp].distrib[ij][0][stage] - FunctGroupArray[sp].distrib[ij][qrt][stage])
                                            + FunctGroupArray[sp].distrib[ij][qrt][stage]);
							...
							if (enviro_depend) {
								...
								if (temp_sensitive_sp && ((current_enviro < min_temp_sp) || (current_enviro > max_temp_sp))) {
									newden[sp][n][k][ij] = 0.0;
								}

								/**
								if((bm->dayt > bm->checkstart) && (sp == bm->which_check)){
									fprintf(llogfp,"Time %e %s-%d box%d-%d enviro_depend: %d newden after enviro %e as current_enviro: %e min_temp_sp: %e max_temp_sp: %e\n",
											bm->dayt, FunctGroupArray[sp].groupCode, n, k, ij, enviro_depend, newden[sp][n][k][ij], current_enviro, min_temp_sp, max_temp_sp);

								}
								**/

								current_enviro = bm->boxes[ij].tr[k][Salinity_i];
								if (salt_sensitive_sp && ((current_enviro < min_salt_sp) || (current_enviro > max_salt_sp))) {
									newden[sp][n][k][ij] = 0.0;
								}

								/**
								if((bm->dayt > bm->checkstart) && (sp == bm->which_check)){
									fprintf(llogfp,"Time %e %s-%d box%d-%d enviro_depend: %d newden after enviro %e as current_enviro: %e min_salt_sp: %e max_salt_sp: %e\n",
											bm->dayt, FunctGroupArray[sp].groupCode, n, k, ij, enviro_depend, newden[sp][n][k][ij], current_enviro,min_salt_sp, max_salt_sp);

								}
								**/

								current_enviro = bm->boxes[ij].tr[k][Oxygen_i];
								if (bm->flagO2depend && (current_enviro < min_O2_sp)) {
									newden[sp][n][k][ij] = 0.0;
								}
			...
			for (ngene = 0; ngene < FunctGroupArray[sp].numGeneTypes; ngene++) {
                ...
                if (bearlive || (localised_recruit == at_parent_location) || (localised_recruit == larval_dispersal)) {
					...
					for (ij = 0; ij < bm->nbox; ij++) {
						...
						if (bm->boxes[ij].type != BOUNDARY){// && bm->boxes[ij].type != LAND) {
							for (k = 0; k < bm->boxes[ij].nz; k++) {
								...
								temp_effect = 1.0;
								current_enviro = bm->boxes[ij].tr[k][Temp_i];
								if (temp_sensitive_sp && ((current_enviro < min_spawntemp_sp) || (current_enviro > max_spawntemp_sp)))
									temp_effect = 0.0;
								
                                salt_effect = 1.0;
								current_enviro = bm->boxes[ij].tr[k][Salinity_i];
								if (salt_sensitive_sp && ((current_enviro < min_spawnsalt_sp) || (current_enviro > max_spawnsalt_sp)))
									salt_effect = 0.0;
                                ...
                                if( update_larval_distrib ) {
                                    for(bcohort = age_mat; bcohort < FunctGroupArray[sp].numCohorts; bcohort++){
                                        ...
                                        totad[ngene] += newden[sp][n][k][ij] * temp_effect * salt_effect * o2_effect * ice_effect;
                                    }
                                    ...
                                    totad[ngene] += bm->recruit_hdistrib_orig[ngene][ij][sp] * temp_effect * salt_effect * o2_effect * ice_effect;
                                }
				...
				if (bm->terrestrial_on == TRUE && bm->boxes[ij].type == LAND) {
					if(FunctGroupArray[sp].habitatCoeffs[LAND_BASED] > 0){
						...
						for (n = 0; n < FunctGroupArray[sp].numGeneTypes; n++) {
							...
							if (bm->norm_larval_distrib && !updated_already && ((bearlive || sp_enviro_depend || (localised_recruit == at_parent_location))
						...
						for (n = 0; n < FunctGroupArray[sp].numCohortsXnumGenes; n++) {
							...
							if (MIGRATION[sp].ActualMigrator[stage] > 0) {
								for (qid = 0; qid < spmigrate; qid++) {
                                    migID = MIGRATION[sp].migrationID[n][qid];
                                    if(migID > (nmigs - 1))
                                        quit("In leave step trying to call a migration ID (%d) that exceeds queue length (%d) for %s\n", migID, nmigs, FunctGroupArray[sp].groupCode);
                                    ...
                                    if(do_debug2){
                                    //if (sp == 44) {
                                        fprintf(bm->logFile, "Time: %e %s-%d box%d-%d leave_stage: %d migID %d (of num_in_queue: %d spmigrate: %d) period: %d\n",
                                                bm->dayt, FunctGroupArray[sp].groupCode, n, ij, k, leave_stage, migID, MIGRATION[sp].num_in_queue, spmigrate, MIGRATION[sp].Leave_Period[leave_stage][migID]);
                                    }
                                    /**/

									if (MIGRATION[sp].Leave_Period[leave_stage][migID] > 0) {
                                        
                                        /**
                                        if(do_debug2){
                                            fprintf(bm->logFile, "Time: %e %s-%d stage: %d Leave_Now = %d period: %d qid: %d\n",
                                                    bm->dayt, FunctGroupArray[sp].groupCode, n, stage, MIGRATION[sp].Leave_Now[leave_stage][qid], MIGRATION[sp].Leave_Period[leave_stage][migID], qid);
                                        }
                                        **/

                                        if ((bm->dayt < MIGRATION[sp].Leave_Now[leave_stage][qid]) && (FunctGroupArray[sp].isComplexMigrator == FALSE))
                                            break;
                                        
										if ((bm->dayt >= MIGRATION[sp].Leave_Now[leave_stage][qid]) && (bm->dayt < (MIGRATION[sp].Leave_Now[leave_stage][qid] + MIGRATION[sp].Leave_Period[leave_stage][migID])) && (bm->t / 86400.0 > 0.0)) {

											migtime2 = Calculate_Migration_Proportion(bm, llogfp, dt, sp,
													MIGRATION[sp].Leave_Now[leave_stage][qid], MIGRATION[sp].Leave_Period[leave_stage][migID], MIGRATION[sp].Box[ij][leave_stage][migID]);

                                            /**
                                            if(do_debug2){
                                                fprintf(bm->logFile, "Time: %e %s-%d migtime2 = %e\n", bm->dayt, FunctGroupArray[sp].groupCode, n, migtime2);
                                            }
                                            **/
											...
											//migProp = MigIOBox[ij][i][stage][sp] * migtime2; /// ( 1.0 + k_migslow * (MIGRATION[sp].Leave_Period[leave_stage][migID] * migtemp - 1.0));
											migProp = migtime2;
											mignum = migProp * newden[sp][n][k][ij];

											/**
											if(do_debug2){
                                                fprintf(bm->logFile, "Time: %e %s-%d (stage %d) Box-%d-%d newden: %e mignum: %e migProp: %e\n",
														bm->dayt, FunctGroupArray[sp].groupCode, n, stage, ij, k, newden[sp][n][k][ij], mignum, migProp);
											}
											**/
											...
											if(newden[sp][n][k][ij] < 0 ){
												...
												warn("away = %d, period = %e, IOBox = %e\n", MIGRATION[sp].Leave_Now[leave_stage][qid], MIGRATION[sp].Leave_Period[leave_stage][migID], MIGRATION[sp].Box[ij][stage][migID]);

												fprintf(stderr, "MigrateAway[%d][%d][%d] = %d\n", sp, stage, qid, MIGRATION[sp].Leave_Now[leave_stage][qid]);
												fprintf(stderr, "MigratePeriod[%d][%d][%d] = %d\n",  sp, stage, qid, MIGRATION[sp].Leave_Period[leave_stage][migID]);
												fprintf(stderr, "MigIOBox[%d][%d][%d][%d]) = %e\n", sp, ij, stage, qid, MIGRATION[sp].Box[ij][stage][migID]);
											}
                                            ...
                                            if(FunctGroupArray[sp].speciesParams[multiyr_mig_id]) {
                                                sp_Migrate_Years = (int) (MIGRATION[sp].YearsAway[stage][migID]);
                                            ...
                                            if (do_debug2) {
                                            //if (sp == 44) {
                                                fprintf(llogfp, "Time: %e %s stage %d migID: %d, multiyr_mig: %d, sp_Migrate_Years: %d, stagger_return: %d, do_stagger: %d totmig: %e as MIG-den: %e mignum: %e, totden: %e\n", bm->dayt, FunctGroupArray[sp].groupCode, stage, migID, (int)(FunctGroupArray[sp].speciesParams[multiyr_mig_id]), sp_Migrate_Years, stagger_return, do_stagger, totmig, MIGRATION[sp].DEN[n][qid], mignum, totden[sp][n]);
                                            }
                                            /**/
                                            
                                            if(do_stagger) {
                                                // Spread return over a few years
                                                stagger_scalar = 1.0 / (FunctGroupArray[sp].speciesParams[stagger_years_id]);
                                                this_mig_num = mignum * totden[sp][n] * stagger_scalar;
                                                
                                                for (nn=0; nn<NumStagger; nn++) {
                                                    kn = n + nn;
                                                    if ( kn > (FunctGroupArray[sp].numCohortsXnumGenes - 1))
                                                        kn = FunctGroupArray[sp].numCohortsXnumGenes - 1;
                                                    
                                                    snn = FunctGroupArray[sp].structNTracers[kn];
                                                    rnn = FunctGroupArray[sp].resNTracers[kn];

                                                    this_totmig = MIGRATION[sp].DEN[kn][qid] + this_mig_num;
                                                    
                                                    MIGRATION[sp].SN[kn][qid] = (MIGRATION[sp].SN[kn][qid] * MIGRATION[sp].DEN[kn][qid] + bm->boxes[ij].tr[k][snn] * this_mig_num) / (this_totmig + small_num);
                                                    MIGRATION[sp].RN[kn][qid] = (MIGRATION[sp].RN[kn][qid] * MIGRATION[sp].DEN[kn][qid] + bm->boxes[ij].tr[k][rnn] * this_mig_num) / (this_totmig + small_num);
                                                    MIGRATION[sp].DEN[kn][qid] = this_totmig;
                                                    
                                                    /**/
                                                    //if (do_debug2) {
                                                    if (sp == 44) {
                                                        fprintf(llogfp, "Time: %e %s-%d stagger qid: %d nn: %d kn: %d totmig: %e totden: %e migProp: %e stagger_scalar: %e migDEN: %e mignum: %e this_mig_num: %e this_totmig: %e SN: %e RN: %e box%d-%d snn: %e rnn: %e\n",bm->dayt, FunctGroupArray[sp].groupCode, n, qid, nn, kn, totmig, totden[sp][n], migProp, stagger_scalar, MIGRATION[sp].DEN[n][qid], mignum, this_mig_num, this_totmig, MIGRATION[sp].SN[n][qid], MIGRATION[sp].RN[n][qid], ij, k, bm->boxes[ij].tr[k][snn], bm->boxes[ij].tr[k][rnn]);
                                                    }
                                                    /**/
                                                ...
                                                if (do_debug2) {
                                                //if (sp == 44) {
                                                    fprintf(llogfp, "Time: %e %s-%d non-stagger qid %d totmig: %e, totden: %e migProp: %e migDEN: %e, mignum: %e SN: %e RN: %e box-%d-%d sn: %e rn: %e\n",bm->dayt, FunctGroupArray[sp].groupCode, n, qid, totmig, totden[sp][n], migProp, MIGRATION[sp].DEN[n][qid], mignum, MIGRATION[sp].SN[n][qid], MIGRATION[sp].RN[n][qid], ij, k, bm->boxes[ij].tr[k][sn], bm->boxes[ij].tr[k][rn]);
                                                }
                                                /**/
                                            }
											...
											/**
											if(do_debug2){
												fprintf(llogfp, "Time: %e, sp %s (cohort %d), totmig = %e, totden[sp][n] = %e\n",bm->dayt, FunctGroupArray[sp].groupCode, n, totmig, totden[sp][n]);
												fprintf(bm->logFile, "migProp = %e, migration = %e, mignum = %e, totden[sp][n]= %e\n", migProp, MIGRATION[sp].DEN[n][qid], mignum, totden[sp][n]);
											}
											**/
											...
											if(do_debug2){
                                            //if (sp == 44) {
												 fprintf(llogfp, "Time: %e, sp %s (cohort %d), totmig = %e, totden[sp][n] = %e migProp = %e, migration = %e, mignum = %e, totden[sp][n]= %e\n",bm->dayt, FunctGroupArray[sp].groupCode, n, totmig, totden[sp][n], migProp, MIGRATION[sp].DEN[n][qid], mignum, totden[sp][n]);
											...
											mig_done++;
										} else if (bm->dayt == (MIGRATION[sp].Leave_Now[leave_stage][qid] + MIGRATION[sp].Leave_Period[leave_stage][migID] + 1)) {
											spupdatemig++;
										}
									}
								}
...
void Ecology_Invert_Migration(MSEBoxModel *bm, double dt, FILE *llogfp) {
	int fgIndex;
	int ij, k, day_part, pid, sp, flagsp, qid, sp_ddepend_move, sp_geo_move, spmigrate, spmigrate_done, end_return_chrt, start_return_chrt, nmigs,
		nc, cohort, flagcontract_sp, stage, maxk, migID = 0, return_stage = 0, leave_stage = 0;
    int temp_sensitive_sp, salt_sensitive_sp;
	double amt, current_enviro, min_temp_sp, max_temp_sp, migperiod, FSM_sp, FSMG_sp, migtime, step1, step2, prop_nc, mignum_nc,
			migbiom, migbiom_basic, FSMG_grow, totamt, min_O2_sp, min_salt_sp, max_salt_sp, contract_sp, pH_scale;
           ...
           if( flagcontract_sp  && (pH_scale > 0.0)) {
            ...
            for (cohort = 0; cohort < FunctGroupArray[fgIndex].numCohortsXnumGenes; cohort++) {
                ...
                for (qid = spmigrate_done; qid < spmigrate; qid++) {
                    ...
                    if(migID > (nmigs - 1))
                        quit("In bring back migrants, trying to call a migration ID (%d) that exceeds queue length (%d) for %s\n", migID, nmigs, FunctGroupArray[sp].groupCode);
                    
                    leave_stage = MIGRATION[sp].leave_stage_prm[cohort][migID]; // Only need on return calculations not leave calculations as just use straight stage then, but messier on return
                    return_stage = MIGRATION[sp].return_stage_prm[cohort][migID];
                    migperiod = MIGRATION[sp].Return_Period[return_stage][migID];
                    start_return_chrt = MIGRATION[sp].start_return_chrt[cohort][qid];
                    end_return_chrt = MIGRATION[sp].end_return_chrt[cohort][qid];
                    FSM_sp = MIGRATION[sp].mortality[return_stage][migID];
                    FSMG_sp = MIGRATION[sp].growth[return_stage][migID];
                            
                    /**
                    if (sp_debug == 2) {
                        fprintf(llogfp, "Time: %e, %s-%d (stage %d) is moving with migrate qid %d: Return_Now: %d, migperiod: %e\n", bm->dayt, FunctGroupArray[sp].groupCode, cohort, stage, qid, MIGRATION[sp].Return_Now[return_stage][qid], migperiod);
                    }
                    **/

                    /* Migration back into to model domain */
                    if ((bm->dayt >= MIGRATION[sp].Return_Now[return_stage][qid]) && (bm->dayt < (MIGRATION[sp].Return_Now[return_stage][qid] + migperiod)) && (bm->t / 86400.0 > 0.0)) {
                        /* Update numbers for returning migrants and then update overall size, based on contributions from returning migrants */
                        migtime = MIGRATION[sp].Return_Now[return_stage][qid] - MIGRATION[sp].Leave_Now[leave_stage][qid];

                        /* Calculate the proportion that are coming back in this timesetp */
                        migtime2 = Calculate_Migration_Proportion_Back(bm, llogfp, dt, sp, MIGRATION[sp].Return_Now[return_stage][qid], migperiod);
                        ...
                        migbiom_basic = MIGRATION[sp].DEN[stage][qid] * migtime2;
                        MIGRATION[sp].DEN[stage][qid] -= migbiom_basic;
                        /* Reset Migration matrix if all have returned or this migration period is over */
                        if (MIGRATION[sp].DEN[stage][qid] < 0.0) {
                            MIGRATION[sp].DEN[stage][qid] = 0.0;
                        }
                        ...
                        /* Save result */
                        step1 = MIGRATION[sp].num_aging_event[cohort][qid] / ((double)(FunctGroupArray[sp].ageClassSize));
                        step2 = step1 - floor(step1);

                        for (nc = start_return_chrt; nc < end_return_chrt; nc++) {
                            prop_nc = (1.0 - step2);

                            mignum_nc = migbiom * prop_nc;
                            totboxden[invert_biosum+nc] += mignum_nc;
                        }

                        if ((bm->dayt + 1) > (MIGRATION[sp].Return_Now[return_stage][qid] + migperiod)) {
                            MIGRATION[sp].DEN[stage][qid] = 0.0;
                        }
                ...
                if (!sp_ddepend_move && sp_geo_move) {
                    ...
                    for (ij = 0; ij < bm->nbox; ij++) {
                        if (bm->boxes[ij].type != BOUNDARY) {
                            ...
                            /* Prescribed movement */
                            if (qrt < (FunctGroupArray[sp].numMoveEntries - 1))
                                newden[sp][cohort][0][ij] = bm->HowFar * (FunctGroupArray[sp].distrib[ij][qrt + 1][cohort] - FunctGroupArray[sp].distrib[ij][qrt][cohort]) + FunctGroupArray[sp].distrib[ij][qrt][cohort];
                            else
                                newden[sp][cohort][0][ij] = bm->HowFar * (FunctGroupArray[sp].distrib[ij][0][cohort] - FunctGroupArray[sp].distrib[ij][qrt][cohort]) + FunctGroupArray[sp].distrib[ij][qrt][cohort];
								
                            /*
                            if(sp_debug == 2){
                                fprintf(llogfp,"day: %e, %s-newden[%d][%d]: %e, HowFar: %e, distrib%s-%d-%d-%d: %e, distrib%s-%d-%d-%d: %e\n",
                                    bm->dayt, FunctGroupArray[sp].groupCode, cohort, ij, newden[sp][cohort][0][ij], bm->HowFar,
                                    FunctGroupArray[sp].groupCode, ij, qrt+1, cohort, FunctGroupArray[sp].distrib[ij][qrt+1][cohort],
                                    FunctGroupArray[sp].groupCode, ij, qrt, cohort, FunctGroupArray[sp].distrib[ij][qrt][cohort]);
                            }
                            */
                            ...
                            */
                            ...
                            if (enviro_depend) {
                                ...
                                for (k = 0; k < maxk; k++) {
                                    ...
                                    current_enviro = bm->boxes[ij].tr[0][Oxygen_i];
                                    if (bm->flagO2depend && (current_enviro < min_O2_sp)) {
                                        newden[sp][cohort][k][ij] = 0.0;
                            ...
                            for (qid = spmigrate_done; qid < spmigrate; qid++) {
                                migID = MIGRATION[sp].migrationID[cohort][qid];
                                if(migID > (nmigs - 1))
                                    quit("In tot up new proportions, trying to call a migration ID (%d) that exceeds queue length (%d) for %s\n", migID, nmigs, FunctGroupArray[sp].groupCode);
                                
                                return_stage = MIGRATION[sp].return_stage_prm[cohort][migID];
                                leave_stage = MIGRATION[sp].leave_stage_prm[cohort][migID];
                                migperiod = MIGRATION[sp].Leave_Period[leave_stage][migID];

                                /* Migration out of model domain */
                                if ((bm->dayt >= MIGRATION[sp].Leave_Now[leave_stage][qid]) && (bm->dayt < (MIGRATION[sp].Leave_Now[leave_stage][qid] + migperiod)) && (bm->t / 86400.0 > 0.0)) {

                                    /* Calculate the proportion that are leaving in this timesetp */
                                    migtime2 = Calculate_Migration_Proportion(bm, llogfp, dt, sp, MIGRATION[sp].Leave_Now[leave_stage][qid], migperiod, MIGRATION[sp].Box[ij][leave_stage][migID]);
                                    ...
                                    /* Update the Migration array to take account of the new
                                        immigrants */
                                    MIGRATION[sp].DEN[stage][qid] += migbiom * totboxden[invert_biosum];
                    ...
                    if (sp_debug == 2) {
                        for (qid = 0; qid < spmigrate; qid++) {
                            fprintf(llogfp, "Time: %e, %s-%d Migration %d Total group biomass outside model is %e\n",
									bm->dayt, FunctGroupArray[sp].groupCode, stage, qid, MIGRATION[sp].DEN[stage][qid] * bm->X_CN * mg_2_tonne);
                        }
                    }
                     */
		...
		for (k = 0; k < bm->boxes[ij].nz; k++) {
			if (bm->flagtempdepend) {
				...
				if ((current_enviro < min_temp_sp) || (current_enviro > max_temp_sp)) {
					...
					if(do_debug){
						fprintf(llogfp,"Time: %e %s box %d-%d day_part: %d, temp-juv: %e (%e), temp-tracker: %e (%e), temp-adult: %e (%e) current_enviro: %e, min_temp: %e, max_temp: %e\n",
							bm->dayt, FunctGroupArray[species].groupCode, ij, k, day_part, tempdistrib[k][juv_id], FunctGroupArray[species].distrib_VERTICAL[day_part][k][juv_id],
								tempdistrib[k][tracker_id], FunctGroupArray[species].distrib_VERTICAL[day_part][k][tracker_id],
								tempdistrib[k][adult_id], FunctGroupArray[species].distrib_VERTICAL[day_part][k][adult_id],
			...
			if (bm->flagsaltdepend) {
				...
				if ((current_enviro < min_salt_sp) || (current_enviro > max_salt_sp)) {
					...
					if(do_debug){
						fprintf(llogfp,"Time: %e %s box %d-%d day_part: %d, temp-juv: %e (%e), temp-tracker: %e (%e), temp-adult: %e (%e) current_enviro: %e, min_salt: %e, max_salt: %e\n",
							bm->dayt, FunctGroupArray[species].groupCode, ij, k, day_part, tempdistrib[k][juv_id], FunctGroupArray[species].distrib_VERTICAL[day_part][k][juv_id],
								tempdistrib[k][tracker_id], FunctGroupArray[species].distrib_VERTICAL[day_part][k][tracker_id],
								tempdistrib[k][adult_id], FunctGroupArray[species].distrib_VERTICAL[day_part][k][adult_id],
			...
			if (bm->flagO2depend) {
				...
				if (current_enviro < min_O2_sp) {
					...
					if(do_debug){
						fprintf(llogfp,"Time: %e %s box %d-%d day_part: %d, temp-juv: %e (%e), temp-tracker: %e (%e), temp-adult: %e (%e) current_enviro: %e, min_O2: %e\n",
							bm->dayt, FunctGroupArray[species].groupCode, ij, k, day_part, tempdistrib[k][juv_id], FunctGroupArray[species].distrib_VERTICAL[day_part][k][juv_id],
								tempdistrib[k][tracker_id], FunctGroupArray[species].distrib_VERTICAL[day_part][k][tracker_id],
								tempdistrib[k][adult_id], FunctGroupArray[species].distrib_VERTICAL[day_part][k][adult_id],
