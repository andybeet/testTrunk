...
void Convert_Groups_To_XML(MSEBoxModel *bm, char *fileName, char *outputFileName) {
	...
	while (fgets(buf, buflen, inputFP) != NULL) {
		...
		if (ch != '\t' && ch != '\n' && ch != '#' && ch != ',') {
			...
			/* VerticallyMigrates */
			varStr = strtok(NULL, seps);
			Util_XML_Create_Node(ATLANTIS_ATTRIBUTE, groupNode, "VerticallyMigrates", "", "", varStr);

			/* HorizontallyMigrates */
			varStr = strtok(NULL, seps);
			Util_XML_Create_Node(ATLANTIS_ATTRIBUTE, groupNode, "HorizontallyMigrates", "", "", varStr);
