...
static double Get_Init_Embryos(MSEBoxModel *bm, int species, int ngene, int stock_id, int qid, int do_debug) {
    ...
    switch (recruit_sp) {
        ...
        case ts_recruit:/* Read in timeseries of recruitment */
            Time_Spawn_sp = FunctGroupArray[species].spawnSpeciesParams[0][Time_Spawn_id];
            this_t = Time_Spawn_sp * 86400.0; // Assume it is equivalent to ehat happens on the first spawn_day
            EMBRYO[species].Larvae[stock_id][ngene][qid] = tsEval(bm->tsRecruit, tsrecruitid, this_t);
            break;
