 ...
 */
static void Update_Detritus(MSEBoxModel *bm, BoxLayerValues *boxLayerInfo, HABITAT_TYPES habitatType, int guild, int processGuild, int cohort) {
	...
	case WC:
		...
		if(bm->track_contaminants){
			...
			Group_Transfer_Contaminant(bm, boxLayerInfo, WC, WC, RefDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDR[cohort]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, WC, WC, LabDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDL[cohort] + FunctGroupArray[guild].lysis[cohort]);

			/* Release contaminants back into the water column */
			Gain_Contaminants(bm, boxLayerInfo, WC, WC, guild, cohort,FunctGroupArray[guild].releaseNH[cohort]);


			/* Now handle transfer due to predation - group gains contaminants due to eating detritus and bacteria.*/
			Group_Transfer_Contaminant(bm, boxLayerInfo, WC, WC, guild, cohort, pelagicBactIndex, 0, GRAZEinfo[pelagicBactIndex][0][WC]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, WC, WC, guild, cohort, RefDetIndex, 0, GRAZEinfo[RefDetIndex][0][WC]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, WC, WC, guild, cohort, LabDetIndex, 0, GRAZEinfo[LabDetIndex][0][WC]);
		}
	...
	case SED:
		...
		if(bm->track_contaminants){
			...
			/* Gains in DR and DL due to mortality */
			Group_Transfer_Contaminant(bm, boxLayerInfo, SED, SED, RefDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDR[cohort]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, SED, SED, LabDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDL[cohort]);

			/* Release contaminants back into the water column */
			Gain_Contaminants(bm, boxLayerInfo, SED, SED, guild, cohort,FunctGroupArray[guild].releaseNH[cohort]);


			/* Contaminant tracer due to predation */
			Group_Transfer_Contaminant(bm, boxLayerInfo, SED, SED, guild, cohort, SedBactIndex, 0, GRAZEinfo[SedBactIndex][0][SED]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, SED, SED, guild, cohort, RefDetIndex, 0, GRAZEinfo[RefDetIndex][0][SED]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, SED, SED, guild, cohort, LabDetIndex, 0, GRAZEinfo[LabDetIndex][0][SED]);
		}
	...
	case EPIFAUNA:
		...
		if(bm->track_contaminants){
			...
			if (FunctGroupArray[processGuild].groupType == SED_EP_OTHER || FunctGroupArray[processGuild].groupType == SED_EP_FF
				...
				Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, SED, RefDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDR[cohort]);
				Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, SED, LabDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDL[cohort]);

				if (FunctGroupArray[guild].diagTol == 2 && it_count == 1){
					/* Release contaminants back into the sediment*/
					Gain_Contaminants(bm, boxLayerInfo, EPIFAUNA, SED, guild, cohort,FunctGroupArray[guild].releaseNH[cohort]);

				}else{
					/* Release contaminants back into the water column*/
					Gain_Contaminants(bm, boxLayerInfo, EPIFAUNA, WC, guild, cohort,FunctGroupArray[guild].releaseNH[cohort]);
				}

			}else{
				/* Contaminants transferred to detritus */
				Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, WC, RefDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDR[cohort]);
				Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, WC, LabDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDL[cohort]);
			...
			Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, WC, guild, cohort, pelagicBactIndex, 0, GRAZEinfo[pelagicBactIndex][0][WC]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, SED, guild, cohort, SedBactIndex, 0, GRAZEinfo[SedBactIndex][0][SED]);

			Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, WC, guild, cohort, RefDetIndex, 0, GRAZEinfo[RefDetIndex][0][WC]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, WC, guild, cohort, LabDetIndex, 0, GRAZEinfo[LabDetIndex][0][WC]);

			Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, SED, guild, cohort, RefDetIndex, 0, GRAZEinfo[RefDetIndex][0][SED]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, SED, guild, cohort, LabDetIndex, 0, GRAZEinfo[LabDetIndex][0][SED]);
		}
		break;
...
int Phytoplankton_Process(MSEBoxModel *bm, FILE *llogfp, HABITAT_TYPES habitatType, int guild, int cohort, BoxLayerValues *boxLayerInfo) {
	...
	if (flag_sp) {
		...
		case WC:
			...
			if(bm->track_contaminants){
				Group_Transfer_Contaminant(bm, boxLayerInfo, WC, WC, LabDetIndex, 0, guild, cohort,  FunctGroupArray[guild].lysis[cohort]);
			}
		...
		case SED:
			...
			if ((int) (FunctGroupArray[guild].speciesParams[flag_id])) {
				...
				if (FunctGroupArray[guild].groupType == MICROPHTYBENTHOS) {
					...
					/* Contaminants transfer into detritus */
					if(bm->track_contaminants){
						Group_Transfer_Contaminant(bm, boxLayerInfo, SED, SED, LabDetIndex, 0, guild, cohort,  FunctGroupArray[guild].lysis[cohort]);
					}
					...
					/* Contaminants transfer into detritus */
					if(bm->track_contaminants){
						Group_Transfer_Contaminant(bm, boxLayerInfo, SED, SED, LabDetIndex, 0, guild, cohort,  FunctGroupArray[guild].mortality[cohort]);
					}
		...
		case EPIFAUNA:
			...
			if (FunctGroupArray[guild].groupType == SEAGRASS) {
				...
				/* Contaminants transfer into detritus */
				if(bm->track_contaminants){
					Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, SED, LabDetIndex, 0, guild, cohort, (FDL * FunctGroupArray[guild].mortality[cohort] / smLayerThick));
					Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, SED, RefDetIndex, 0, guild, cohort, ((1 - FDL) * FunctGroupArray[guild].mortality[cohort] / smLayerThick));
				}
				...
				/* Contaminants transfer into detritus */
				if(bm->track_contaminants){
					Group_Transfer_Contaminant(bm, boxLayerInfo, EPIFAUNA, SED, LabDetIndex, 0, guild, cohort,  FunctGroupArray[guild].mortality[cohort] / wcLayerThick);
				}
...
//			boxLayerInfo->NutsLost[habitatType][Si_id] += uptakeSi;
			...
			/* Contaminants transfer into detritus */
			if(bm->track_contaminants){
				Group_Transfer_Contaminant(bm, boxLayerInfo, ICE_BASED, ICE_BASED, LabDetIndex, 0, guild, cohort, (FunctGroupArray[guild].lysis[cohort] + FunctGroupArray[guild].mortality[0]));
			}
...
int Invert_Consumers_Process(MSEBoxModel *bm, FILE *llogfp, HABITAT_TYPES habitatType, int guild, int cohort, BoxLayerValues *boxLayerInfo) {
    ...
    //        current_depth >= (-1.0 * FunctGroupArray[guild].speciesParams[maxdepth_id]))) {
		...
		if (habitatType == WC || FunctGroupArray[guild].groupType == CEP) {
			Update_Detritus(bm, boxLayerInfo, WC, guild, guild, cohort);
			Update_Debug_Info(bm, boxLayerInfo, WC, guild, cohort);
		} else if (habitatType == SED) {
			Update_Detritus(bm, boxLayerInfo, SED, guild, guild, cohort);
			Update_Debug_Info(bm, boxLayerInfo, SED, guild, cohort);
		} else {
			Update_Detritus(bm, boxLayerInfo, EPIFAUNA, guild, guild, cohort);
		...
		if (bm->debug == debug_prey_biology_process && bm->dayt >= bm->checkstart && bm->dayt < bm->checkstop) {
			...
			fprintf(llogfp,
					"Invert_Consumers_Process %s, eatBiomass = %.20e, - outcome DetritusProd[SED][DLdet_id = %.20Le, NutsProd[SED][NH_id] = %.20Le\n",
...
int Carrion_Process(MSEBoxModel *bm, FILE *llogfp, HABITAT_TYPES habitatType, int guild, int cohort, BoxLayerValues *boxLayerInfo) {
	...
	if (it_count == 1) {
		...
		if(bm->track_contaminants){
			...
			Group_Transfer_Contaminant(bm, boxLayerInfo, habitatType, habitatType, LabDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDL[0]);
			Group_Transfer_Contaminant(bm, boxLayerInfo, habitatType, habitatType, RefDetIndex, 0, guild, cohort, FunctGroupArray[guild].prodnDR[0]);
		}
