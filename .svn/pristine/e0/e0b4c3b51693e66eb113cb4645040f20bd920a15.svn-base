    ...
    if (flag_sp && active_sp && do_level) {
		...
		for (cohort = 0; cohort < FunctGroupArray[guildcase].numCohortsXnumGenes; cohort++) {
            if ((spSPinfo[guildcase][cohort][SN_id] > bm->min_pool) && (spSPinfo[guildcase][cohort][DEN_id] > bm->min_dens)) {
                ...
                if ((FRC_V >= 0.0) || !bm->flag_shrinkfat) {
                    if (bm->flag_dynamicXRS) {
                ...
                **/
				...
				if(bm->track_contaminants){
					...
					Group_Transfer_Contaminant(bm, boxLayerInfo, WC, WC, LabDetIndex, 0, guildcase, cohort, VprodDLchrt);
					Group_Transfer_Contaminant(bm, boxLayerInfo, WC, WC, RefDetIndex, 0, guildcase, cohort, VprodDRchrt);

					/* Release contaminants back into the water column */
					Gain_Contaminants(bm, boxLayerInfo, WC, WC, guildcase, cohort, VreleaseNHchrt);
				}
