        ...
        for(migrationID = 0; migrationID < FunctGroupArray[sp].speciesParams[num_migrate_id]; migrationID++){
            ...
            for(migID = 0; migID < FunctGroupArray[sp].speciesParams[num_migrate_id]; migID++){
                ...
                if (sp_Migrate_Time > sp_Migrate_Return_chk) {
                     ...
                     */
                    warn("The migration entry %d (starting %d) for %s-%d happens completely before migration entry %d (starting %d). Think about fixing that in the parameter file as it will help the code run faster\n", migrationID, sp_Migrate_Time, FunctGroupArray[sp].groupCode, stage, migID, sp_Migrate_Return_chk);
...
void Check_For_Mature_Away(MSEBoxModel *bm, FILE *llogfp, int species, int time_away, int YearsAway, int migrationID, int currentID){
    ...
    if (!bm->flag_multiyr_migs || (YearsAway <= 1)) {  /* TODO: This may need to change if spawn more than once per year */
        // The rest is the same way it always was so no more to do
    } else {
        ...
        for(cohort = 0; cohort < FunctGroupArray[species].numCohortsXnumGenes; cohort++){
            ...
            for (counter = currentID; counter < MIGRATION[species].num_in_queue; counter++) {
                ...
                if (MIGRATION[species].end_return_chrt[cohort][counter] > FunctGroupArray[species].numCohortsXnumGenes)
                    MIGRATION[species].end_return_chrt[cohort][counter] = FunctGroupArray[species].numCohortsXnumGenes;  // TODO: This may need a fix for multi-gene type cases
                
                //fprintf(llogfp,"Check_For_Mature_Away %s-%d counter %d has start_return_chrt: %d end_return_chrt: %d\n", FunctGroupArray[species].groupCode, cohort, counter, MIGRATION[species].start_return_chrt[cohort][counter], MIGRATION[species].end_return_chrt[cohort][counter]);
            }
...
void Init_Migration_Age_Check(MSEBoxModel *bm, FILE *llogfp, int do_debug, int sp) {
    ...
    if (this_stagger_return && !FunctGroupArray[sp].speciesParams[stagger_years_id])
        quit("For %s StaggerYears cannot be set to 0 if FlagMultiyrMigType is set > 0, please fix the parametersation\n", FunctGroupArray[sp].groupCode);
    ...
    for(cohort = 0; cohort < FunctGroupArray[sp].numCohortsXnumGenes; cohort++){
        ...
        // Then depending on ageclasssize determine how split up recruits will be
            ...
            if (end_return_chrt > MIGRATION[sp].end_return_chrt[cohort][qid]) {
                ...
                if (MIGRATION[sp].end_return_chrt[cohort][qid] > FunctGroupArray[sp].numCohortsXnumGenes)
                    MIGRATION[sp].end_return_chrt[cohort][qid] = FunctGroupArray[sp].numCohortsXnumGenes;  // TODO: This may need a fix for multi-gene type cases
            }

            //fprintf(llogfp,"Init_Migration_Age_Check %s-%d start_return_chrt: %d end_return_chrt: %d numCohortsXnumGenes: %d numCohorts: %d numGeneTypes: %d ngene: %d\n", FunctGroupArray[sp].groupCode, cohort, MIGRATION[sp].start_return_chrt[cohort][qid], MIGRATION[sp].end_return_chrt[cohort][qid], FunctGroupArray[sp].numCohortsXnumGenes, FunctGroupArray[sp].numCohorts, FunctGroupArray[sp].numGeneTypes, ngene);
