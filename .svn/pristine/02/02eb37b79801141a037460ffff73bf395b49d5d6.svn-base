...
void Set_Active_Flag(MSEBoxModel *bm, FILE *llogfp) {
	...
	for (ij = 0; ij < nTrSize; ij++) {
		...
		if (Bioflag[id]) {
			...
			/* If the tracer is not found make it in active */
			if(checkdone < 0){
				Activeflag[id] = 0;
			...
			if (flag_sp) {
				...
				for (n = 0; n < FunctGroupArray[sp].numCohortsXnumGenes; n++) {
					...
					for (qid = 0; qid < MIGRATION[sp].num_in_queue; qid++) {
                        ...
                        bm->totbiom[sp] += MIGRATION[sp].mortality[stage][migID] * (MIGRATION[sp].DEN[n][qid] * (MIGRATION[sp].SN[n][qid]
								+ MIGRATION[sp].RN[n][qid]));
						bm->totagepop[sp][n] += MIGRATION[sp].mortality[stage][migID] * (MIGRATION[sp].DEN[n][qid] * (MIGRATION[sp].SN[n][qid]
						        + MIGRATION[sp].RN[n][qid]));

						/**
						fprintf(llogfp,"%s-%d qid: %d, stage: %d, totbiom: %f, FSM: %f, DEN: %f, SN: %f, RN: %f\n", 
							FunctGroupArray[sp].groupCode, n, qid, stage, bm->totbiom[sp], MIGRATION[sp].mortality[stage][migID], MIGRATION[sp].DEN[n][qid], MIGRATION[sp].SN[n][qid], MIGRATION[sp].RN[n][qid]);
						**/

                        sp_abund += MIGRATION[sp].DEN[n][qid];

						if (n == fished_chrt) {
							bm->calcNstart[sp][hist_id] += MIGRATION[sp].mortality[stage][migID] * MIGRATION[sp].DEN[n][qid];
						}

						bm->calcNstartPerPred[sp][hist_id] += MIGRATION[sp].mortality[stage][migID] * MIGRATION[sp].DEN[n][qid];
		...
		if (FunctGroupArray[fgIndex].isVertebrate == FALSE && FunctGroupArray[fgIndex].isMobile == TRUE && FunctGroupArray[fgIndex].sp_geo_move == TRUE) {
			for (cohort = 0; cohort < FunctGroupArray[fgIndex].numCohortsXnumGenes; cohort++) {
				...
				for (qid = 0; qid < MIGRATION[fgIndex].num_in_queue; qid++) {
					bm->totbiom[fgIndex] += MIGRATION[fgIndex].mortality[stage][migID] * MIGRATION[fgIndex].DEN[cohort][qid];
					bm->totagepop[fgIndex][cohort] += MIGRATION[fgIndex].mortality[stage][migID] * MIGRATION[fgIndex].DEN[cohort][qid];
				}
			}
		}
	}
...
void Allocate_Arrays_Post_Load(MSEBoxModel *bm, FILE *llogfp) {
	...
	BED_scale = Util_Alloc_Init_1D_Double(K_num_sed_type, 1.0);
	...
	shiftVERTON = Util_Alloc_Init_1D_Int(bm->K_num_tot_sp, 0);
