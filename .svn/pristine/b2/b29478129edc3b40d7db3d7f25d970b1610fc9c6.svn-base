...
void createMigrationXML(MSEBoxModel *bm, FILE *fp, char *fileName, xmlDocPtr doc, xmlNodePtr rootnode) {
	...
	Create_Species_ParamXML(bm, fileName, fp, groupingNode, num_migrate_id,
			"Flags for species migration out of model domain (enter juveniles and adults separately for age structured groups), 0=always remain within the model domain, 1+=migrate in/out of the model",
			"", XML_TYPE_INTEGER, "0");
    
    if(bm->flag_multiyr_migs) {
        Create_Species_ParamXML(bm, fileName, fp, groupingNode, multiyr_mig_id,
            "Flags for species multiyear migration out of model domain: 0=always remain within the model domain or only migrate annually, 1+=migrate in/out of the model over periods of multiple years",
            "", XML_TYPE_INTEGER, "0");
        
        Create_Species_ParamXML(bm, fileName, fp, groupingNode, flag_multiyr_type_id, "Flag indicating what type of multi-year migration is occuring - whether serial or synchronous", "", XML_TYPE_INTEGER,"0");
        
        Create_Species_ParamXML(bm, fileName, fp, groupingNode, stagger_years_id, "Number of years that stagger return over", "", XML_TYPE_FLOAT, "0");
	...
	Parse_File(bm, fp, fileName, groupingNode, "MigrateReturn", "_Migrate_Return$",
			"Day of year the functional group returns to the model domain. Array of size NUM_MIGRATIONS for each migrating group", "day",
			XML_TYPE_INTEGERARRAY, -1,
			FALSE, InitMigrateXML, MigrateReturnXMLFunction);
	Parse_File(bm, fp, fileName, groupingNode, "MigratePeriod", "_Migrate_Period$",
			"Period of time migrating group exits/enters over, Array of size NUM_MIGRATIONS for each migrating group", "days", XML_TYPE_INTEGERARRAY, -1, FALSE,
			InitMigrateXML, MigrateReturnXMLFunction);
	Parse_File(bm, fp, fileName, groupingNode, "MigrateTime", "_Migrate_Time$",
			"Day of year the functional group leaves to the model domain. Array of size NUM_MIGRATIONS for each migrating group", "day", XML_TYPE_INTEGERARRAY, -1,
			FALSE, InitMigrateXML, MigrateReturnXMLFunction);

    if (bm->flag_multiyr_migs)
        Parse_File(bm, fp, fileName, groupingNode, "MigrateYearsAway", "_Migrate_YearsAway$",
                   "Number of years the migrating group spends outside the model domain, Array of size NUM_MIGRATIONS for each migrating group", "days", XML_TYPE_INTEGERARRAY, -1, FALSE, InitMigrateXML, MigrateReturnXMLFunction);
	...
	Parse_File( bm, fp, fileName, groupingNode, "MigrateStockReturn", "_ReturnStock$",
			"Migration return stock - set value to the integer number identifying the stock the immigrating individuals enter. If set to 0 then the returning individuals will be spread uniformly across all stocks. Array of size NUM_MIGRATIONS for each migrating group.",
			"", XML_TYPE_INTEGERARRAY, -1, FALSE, InitStockMigrateXML, MigrateReturnXMLFunction);
	Parse_File(bm, fp, fileName, groupingNode, "MigrateProportionIncrease", "_FSMG$",
			"Proportional increases in size while outside model domain. Array of size NUM_MIGRATIONS for each migrating group", "",
			XML_TYPE_INTEGERARRAY, -1, FALSE, InitMigrateXML, MigrateReturnXMLFunction);
	Parse_File( bm, fp, fileName, groupingNode, "MigrateSurvivorshipProportion", "_FSM$",
			"external/migration survivorship - proportion of migrating biomass that return to the model domain. Array of size NUM_MIGRATIONS for each migrating group",
			"", XML_TYPE_INTEGERARRAY, -1, FALSE, InitMigrateXML, MigrateReturnXMLFunction);
...
void createDietXML(MSEBoxModel *bm, FILE *fp, char *fileName, xmlDocPtr doc, xmlNodePtr rootnode) {
	...
	if(bm->flag_macro_model){
		Parse_File(bm, fp, fileName, groupingNode, "SeagrassFoodAvail", "^pSGPREY[A-Z]", "Seagrass food availability", "", XML_TYPE_FLOATARRAY, num_macrophyte_indices, TRUE, InitspSeagrassXML, SeagrassPreyXMLFunction);
	}
	...
	Parse_File(bm, fp, fileName, groupingNode, "ClearanceRate", "^C_[A-Z]{2,3}$", "Clearance Rate", "mg3(mg N)-1 d-1", XML_TYPE_FLOATARRAY, bm->K_num_max_cohort, TRUE, Init_Vert_Species_Zero_ValuesXML, Species_Last_XMLFunction);
	...
	Parse_File_Single_Line(bm, fp, fileName, groupingNode, "InvertebrateClearanceRate", "^C_.*_T15$", "Invertebrate Clearance Rate", "mg3(mg N)-1 d-1",
			XML_TYPE_FLOATARRAY, "0.0002 - 0.4", TRUE, InitInvertSpeciesZeroValuesXML, InvertebrateClearanceRateXMLFunction);
	...
	Parse_File_Single_Line(bm, fp, fileName, childGroupingNode, "InvertebrateGrowthRate", "^mum_.*_T15$",
			...
			InitInvertSpeciesZeroValuesXML, InvertebrateClearanceRateXMLFunction);
