	...
	*dtsz = RelTol / max_del;
	...
	if (*dtsz < 0.1) {
        ...
        if (bm->boxes[bm->current_box].relax_tol){
            fprintf(llogfp, "Time step size %e (seconds) is too small, but in box 3 so lets keep going!\n", *dtsz);
	...
	for (ij = 0; ij < numVariables; ij++) {
		...
		**/
		...
		if(!(_finite(tracerArray[ij]))) {
			quit("Integrate_Tracer_Variables tracer %d - %s is nan in box %d:layer %d, it_count = %d (flux: %e, tsz: %e)\n",
					ij, Varname[offset + ij], tracerArray[ij],bm->current_box, bm->current_layer, it_count, fluxArray[ij], tsz);
		}
		if (it_count == 1) {
		...
		if (tracerArray[ij] < bm->min_pool) {
			...
			if (!bm->do_availflag || !Bioflag[offset + ij] || (bm->do_availflag && Bioflag[offset + ij] && Activeflag[offset + ij])) {
...
static void Copy_SED_Tracers(MSEBoxModel *bm, double *localTracers, double *localFlux) {
	...
	for (i = 0; i < bm->ntracer; i++) {
		...
		if (!(_finite(localTracers[i]))) {
			quit("Copy_SED_Tracers - day: %e, box: %d, layer: %d %s localpool set to: %e.\n", bm->dayt, bm->current_box, bm->current_layer, Varname[i],
					localTracers[i]);
		}
