...
void Check_Gape(MSEBoxModel *bm, FILE *llogfp) {
	...
	int ij, k, n, sn, rn, den, sp, prey, preyage, chrtstage, preychrt, flag_sp, maxstock_id, i, indx, qidi, return_stage;
	int qid, recdate, spmigrate, pid, migID;
				...
				if (flag_sp) {
                    ...
                    for (n = 0; n < FunctGroupArray[sp].numCohortsXnumGenes; n++) {
                        ...
                        for (qid = MIGRATION[sp].num_in_queue_done; qid < MIGRATION[sp].num_in_queue; qid++) {
                            ...
                            //printf("Last step Check_Gape W-%d-%d-d %s qid: %d migID: %d return_stage: %d\n", sp, n, FunctGroupArray[sp].groupCode, qid, migID, return_stage);
                            
                            if (spmigrate && (bm->dayt >= MIGRATION[sp].Leave_Now[pid][qid]) && (bm->dayt < MIGRATION[sp].Return_Now[return_stage][qid] + MIGRATION[sp].Period[return_stage][migID])){
                                ...
                                //printf("Last step Check_Gape W-%d qid: %d %s, cohort %d - migration %d underway (relsn: %e relrn: %e relden: %e as sn: %e rn: %e den: %e and return %d)\n", sp, qid, FunctGroupArray[sp].groupCode, n, qid, relSN, relRN, relDEN, MIGRATION[sp].SN[n][qid], MIGRATION[sp].RN[n][qid], MIGRATION[sp].DEN[n][qid], MIGRATION[sp].Return_Now[return_stage][qid]);

                                fprintf(llogfp, "%s, cohort %d - migration %d underway (relsn: %e relrn: %e relden: %e as sn: %e rn: %e den: %e and return %d)\n", FunctGroupArray[sp].groupCode, n, qid, relSN, relRN, relDEN, MIGRATION[sp].SN[n][qid], MIGRATION[sp].RN[n][qid], MIGRATION[sp].DEN[n][qid], MIGRATION[sp].Return_Now[return_stage][qid]);
                            }
						}
